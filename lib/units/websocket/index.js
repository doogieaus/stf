...
module.exports = function(options) {
  ...
  io.on('connection', function(socket) {
    ...
    var messageListener = wirerouter()
      ...
      .on(wire.NetworkConnectivityEvent, function(channel, message) {
        socket.emit('device.change', {
          important: false
        , data: {
            serial: message.serial
    ...
    new Promise(function(resolve) {
      ...
      socket.on('disconnect', resolve)
        ...
        // Device market name
        .on('device.marketName', function(data) {
          return dbapi.setDeviceMarketName(data.serial, data.marketName)
            .then(function() {
              return dbapi.loadDevice(user.groups.subscribed, data.serial)
            })
            .then(function(cursor) {
