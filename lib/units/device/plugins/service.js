      ...
      if (envelope.id !== null) {
        ...
        switch (envelope.type) {
          ...
          case apk.wire.MessageType.EVENT_ROTATION:
            ...
            break
          case apk.wire.MessageType.EVENT_NETWORK_CONNECTIVITY:
            message = apk.wire.NetworkConnectivityEvent.decode(envelope.message)
            log.info('Received network connectivity change message')
            
            // Override the ipAddress list
            var ipAddressList = { }
            message.ipAddressList.split(';').forEach(element => {
            ...
            if ('wlan0' in ipAddressList) {
              if ('ipv6' in ipAddressList.wlan0) {
                ipAddress = ipAddressList.wlan0.ipv6
              }
              if ('ipv4' in ipAddressList.wlan0) {
                ipAddress = ipAddressList.wlan0.ipv4
              }
            }

            push.send([
              wireutil.global
              , wireutil.envelope(new wire.NetworkConnectivityEvent(
                options.serial
                , ipAddress
              ))
            ])
            plugin.emit('networkConnectivityChange', message)
            break
        }
      }
    }
