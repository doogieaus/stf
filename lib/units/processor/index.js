...
module.exports = db.ensureConnectivity(function(options) {
  ...
  devDealer.on('message', wirerouter()
    ...
    .on(wire.DeviceIdentityMessage, function(channel, message, data) {
      // Make sure we don't override the marketName
      dbapi.getDevice(message.serial).then(function(device) {
        if (device) {
          message.marketName = device.marketName
        }
    ...
    .on(wire.NetworkConnectivityEvent, function(channel, message, data) {
      dbapi.setDeviceIpAddress(message.serial, message.ipAddress)
      appDealer.send([channel, data])
    })
