...
dbapi.setDeviceIpAddress = function(serial, ipAddress) {
  return db.run(r.table('devices').get(serial).update({
    phone: {
      ipAddress: ipAddress
    }
...
dbapi.getDevice = function(serial) {
  return db.run(r.table('devices').get(serial))
}

dbapi.setDeviceMarketName = function(serial, marketName) {
  return db.run(r.table('devices').get(serial).update({
...
dbapi.saveDeviceIdentityWithoutMarketName = function(serial, identity) {
  return db.run(r.table('devices').get(serial).update({
    platform: identity.platform
  , manufacturer: identity.manufacturer
  , operator: identity.operator
